...
class Predictor(DefaultPredictor):
    ...
    def format_results(self, class_names):
        ...
        masks = [GenericMask(x, v.output.height, v.output.width)
                 for x in masks]
...
class DetectedObject:
    ...
    def create_vector_arrow(self):
        ...
        """
        arrow_ratio = AXES_SIZE / max(abs(self.track.velocity_vector[0]), abs(
            self.track.velocity_vector[1]), abs(self.track.velocity_vector[2]))
        self.track.v_points = [
            x * arrow_ratio for x in self.track.velocity_vector]
...
def debug_plots(color_image, depth_image, mask, histg, depth_colormap):
    ...
    """
    full_hist = cv2.calcHist([depth_image], [0], None, [
                             NUM_BINS], [0, MAX_RANGE])
    masked_depth_image = cv2.bitwise_and(
        depth_colormap, depth_colormap, mask=mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--file', help='type --file=file-name.bag to stream using file instead of webcam')
    ...
    while True:
        ...
        v = OptimizedVisualizer(
            color_image[:, :, ::-1], MetadataCatalog.get(predictor.config.DATASETS.TRAIN[0]))
        ...
        for i in range(num_masks):
            ...
            histg = cv2.calcHist([depth_image], [0], detected_objects[i].mask.mask, [
                                 NUM_BINS], [0, MAX_RANGE])
            ...
            track = mot_tracker.matched[np.where(
                mot_tracker.matched[:, 0] == i)[0], 1]
                ...
                if i not in mot_tracker.unmatched:
                    try:
                        ...
                        if hasattr(mot_tracker.trackers[track], 'distance'):
                            mot_tracker.trackers[track].set_speed(
                                centre_depth, total_speed_time)

                            mot_tracker.trackers[track].set_impact_time(
                                centre_depth)

                            if mot_tracker.trackers[track].impact_time != False and mot_tracker.trackers[track].impact_time >= 0:
                                v.draw_text("{:.2f} seconds to impact".format(
                                    mot_tracker.trackers[track].impact_time), (cX, cY + 60))
                        ...
                        if hasattr(mot_tracker.trackers[track], 'position'):
                            ...
                            x1, y1, z1 = rs.rs2_deproject_pixel_to_point(
                                video_streamer.depth_intrin, [
                                    cX, cY], centre_depth
                            )

                            # Update states for tracked object
                            mot_tracker.trackers[track].set_velocity_vector(
                                x1, y1, z1)
                            mot_tracker.trackers[track].set_distance_3d(
                                x1, y1, z1)
                            mot_tracker.trackers[track].set_velocity(
                                total_speed_time)

                            detected_objects[i].track = mot_tracker.trackers[track]

                            v.draw_text(
                                "{:.2f}m/s".format(detected_objects[i].track.velocity), (cX, cY + 40))
